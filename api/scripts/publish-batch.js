...
import { publishScriptsBatch } from "../_lib/services/scripts.js";
...
export default async function handler(req, res) {
  ...
  try {
    return res.status(200).json(await publishScriptsBatch(ids));
  } catch (err) {
    return handleServerError(res, err, "Batch publish scripts");
  }
}
